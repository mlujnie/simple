...
Plot plt.loglog(Ls, lim.luminosity_function(Ls)) in a reasonable range to check the outcome!"""
        ...
        try:
            ...
            self.run_corrfunc = input_dict["run_corrfunc"]
        except:
            self.run_pk = {
                "intensity": False,
                "n_gal": False,
    ...
    def Pk_2d(self, tracer="intensity", save=False):
        ...
        if tracer == "intensity":
            ...
            unit_factor = intensity_map_unit**2
        elif tracer == "n_gal":
            fftpower = FFTPower(
                galaxy_map_to_use,
                mode="2d",
            ...
            unit_factor = galaxy_map_unit**2
        else:  # cross power spectrum
            fftpower = FFTPower(
                first=intensity_map_to_use,
                second=galaxy_map_to_use,
                mode="2d",
                Nmu=self.N_mu,
                poles=[0, 2],
                los=self.LOS,
            ...
            unit_factor = intensity_map_unit * galaxy_map_unit

        monopole = (fftpower.poles["power_0"] * self.Mpch**3).to(self.Mpch**3)
        quadrupole = (fftpower.poles["power_2"] *
                      self.Mpch**3).to(self.Mpch**3)
        P_k_mu = (fftpower.power["power"] * self.Mpch**3).to(self.Mpch**3)
        mu_wedges = fftpower.power.coords["mu"]
        k_bins = fftpower.poles["k"]
        ...
        if save:
            ...
            with h5py.File(outfilename, "a") as ff:
                ...
                grp = ff.create_group(tracer)
                ff[f"{tracer}/monopole"] = monopole.to(self.Mpch**3).value
                ff[f"{tracer}/quadrupole"] = quadrupole.to(self.Mpch**3).value
                ff[f"{tracer}/P_k_mu"] = P_k_mu.to(self.Mpch**3).value
                ff[f"{tracer}/mu_wedges"] = mu_wedges
                ff[f"{tracer}/k_bins"] = k_bins
                for key in ["monopole", "quadrupole", "P_k_mu"]:
                    ff[f"{tracer}/{key}"].attrs["unit"] = str(self.Mpch**3)
            logging.info(f"Saved to {outfilename}")

        logging.info("Done.")
        return k_bins, mu_wedges, P_k_mu, monopole, quadrupole
    ...
    def prepare_intensity_and_randoms(
        self, tracer, new_N_mesh=None, new_voxel_length=None, with_noise=True
    ):
        # TODO: add choice of all/detected/undetected galaxies
        if tracer == "intensity":
            mesh_to_use = self.intensity_mesh.to(self.mean_intensity).value
            if with_noise:
                mesh_to_use += self.noise_mesh.to(self.mean_intensity).value
        elif tracer == "n_gal":
            mesh_to_use = self.n_gal_mesh.to(u.Mpc ** (-3)).value
        else:
            raise ValueError(
                "Impermissible argument: Parameter 'tracer' has to be either 'intensity' or 'n_gal'."
            )

        if not (new_voxel_length is None):
            new_N_mesh = (self.box_size / new_voxel_length).to(1).astype(int)

        # if I use !=, you get a boolean array.
        MASK = self.obs_mask is not None
        if MASK:
            logging.info("Preparing everything with a mask.")

        if new_N_mesh is None:
            mesh_positions = self.get_mesh_positions(
            ...
            if MASK:
                intensity = np.concatenate(
                    np.concatenate(mesh_to_use * self.obs_mask))
                mean_intensity_per_redshift_mesh = np.concatenate(
                    np.concatenate(
                        self.obs_mask *
                        mean_intensity_per_redshift[:, None, None]
                    )
                )
            else:
                intensity = np.concatenate(np.concatenate(mesh_to_use))
                mean_intensity_per_redshift_mesh = np.concatenate(
                    np.concatenate(
                        np.ones(mesh_to_use.shape)
                        * mean_intensity_per_redshift[:, None, None]
                    )
                )
        else:
            new_N_mesh = np.array(new_N_mesh)
            mesh_positions = self.get_mesh_positions(
                self.box_size / new_N_mesh, new_N_mesh
            )
            downsampled_intensity = self.downsample_mesh(
                make_map(
                    mesh_to_use,
                    Nmesh=self.N_mesh,
                    BoxSize=self.box_size.to(self.Mpch).value,
                ),
                new_N_mesh=new_N_mesh,
            )
            distances = (
                self.minimum_distance
                + self.box_size[0] / new_N_mesh[0] *
                (np.arange(new_N_mesh[0]) + 0.5)
            ).to(u.Mpc)
            ...
            if MASK:
                ...
                downsampled_intensity = downsampled_intensity * downsampled_mask
                mean_intensity_per_redshift_mesh = np.concatenate(
                    np.concatenate(
                        np.array(downsampled_mask)
                        * mean_intensity_per_redshift[:, None, None]
                    )
                )
            else:
            ...
            intensity = np.concatenate(
                np.concatenate(np.array(downsampled_intensity)))
        if tracer == "intensity":
            mean_intensity_per_redshift_mesh = mean_intensity_per_redshift_mesh.to(
                self.mean_intensity
            ).value
        elif tracer == "n_gal":
            mean_intensity_per_redshift_mesh = mean_intensity_per_redshift_mesh.to(
        ...
        mesh_positions = np.concatenate(
            np.concatenate(mesh_positions.to(self.Mpch).value)
        )
        return mesh_positions * self.Mpch, intensity, mean_intensity_per_redshift_mesh

    def calculate_1d_correlation_function(
        self,
        tracer1="intensity",
        tracer2=None,
        new_N_mesh=None,
        new_voxel_length=None,
        bin_type="Log",
        min_sep=10,
        max_sep=200,
        nbins=10,
    ):
        mesh_positions, data_first, random_first = self.prepare_intensity_and_randoms(
            tracer=tracer1, new_N_mesh=new_N_mesh, new_voxel_length=new_voxel_length
        )
        if tracer2 is not None:
            logging.info(
                f"Calculating cross-correlation function of {tracer1} and {tracer2}."
            )
            (
                mesh_positions_second,
                data_second,
                random_second,
            ) = self.prepare_intensity_and_randoms(
                tracer=tracer2, new_N_mesh=new_N_mesh, new_voxel_length=new_voxel_length
            )
        else:
            logging.info(
                f"Calculating auto-correlation function of {tracer1}.")

        logging.info(
            "Mean ratio data/random: {}".format(
                np.mean(data_first) / np.mean(random_first)
            )
        )

        mesh_positions = mesh_positions.to(self.Mpch).value
        x, y, z = (
            mesh_positions[:, 0],
            mesh_positions[:, 1],
            mesh_positions[:, 2],
        )  # in Mpc/h
        logging.info("mesh_positions.shape: {}".format(mesh_positions.shape))
        logging.info("{} {} {} {}".format(
            x.shape, y.shape, z.shape, data_first.shape))
        data_first_catalog = treecorr.Catalog(x=x, y=y, z=z, k=data_first)
        random_first_catalog = treecorr.Catalog(x=x, y=y, z=z, k=random_first)
        if tracer2 is not None:
            data_second_catalog = treecorr.Catalog(
                x=x, y=y, z=z, k=data_second)
            random_second_catalog = treecorr.Catalog(
                x=x, y=y, z=z, k=random_second)

        treecorr_config = {
            "bin_type": bin_type,
            "min_sep": min_sep,  # lim.voxel_length.to(lim.Mpch).value[0],
            "max_sep": max_sep,
            "nbins": nbins,
            "metric": "Euclidean"
            # 'bin_slop': 0.
        ...
        if tracer2 is None:
            DD = treecorr.KKCorrelation(treecorr_config)
            DR = treecorr.KKCorrelation(treecorr_config)
            RR = treecorr.KKCorrelation(treecorr_config)
            logging.info("Done initializing correlation function.")
            DD.process(data_first_catalog)
            logging.info("Done processing DD correlation function.")
            DR.process(data_first_catalog, random_first_catalog)
            logging.info("Done processing DR correlation function.")
            RR.process(random_first_catalog)
            logging.info("Done processing RR correlation function.")

            final_correlation = (DD.xi - 2 * DR.xi + RR.xi) / RR.xi
        else:
            DD = treecorr.KKCorrelation(treecorr_config)
            D1R2 = treecorr.KKCorrelation(treecorr_config)
            D2R1 = treecorr.KKCorrelation(treecorr_config)
            RR = treecorr.KKCorrelation(treecorr_config)
            logging.info("Done initializing correlation function.")
            DD.process(data_first_catalog, data_second_catalog)
            D1R2.process(data_first_catalog, random_second_catalog)
            D2R1.process(random_first_catalog, data_second_catalog)
            RR.process(random_first_catalog, random_second_catalog)
            logging.info("Done processing the correlation function.")

            final_correlation = (DD.xi - D1R2.xi - D2R1.xi + RR.xi) / RR.xi

        return DD.meanlogr, final_correlation

    def calculate_2d_correlation_function(
        self,
        tracer="intensity",
        tracer2=None,
        new_N_mesh=None,
        ...
        logging.info(f"Calculating 2D correlation function of {tracer}.")
        ...
        (
            ...
            tracer=tracer, new_N_mesh=new_N_mesh, new_voxel_length=new_voxel_length
        )
        if tracer2 is not None:
            (
                position_vector,
                intensity2,
                mean_intensity_per_redshift_mesh2,
            ) = self.prepare_intensity_and_randoms(
                tracer=tracer2, new_N_mesh=new_N_mesh, new_voxel_length=new_voxel_length
            )
        else:
            intensity2 = intensity
            mean_intensity_per_redshift_mesh2 = mean_intensity_per_redshift_mesh
        # position_vector: scalar in units of Mpc/h
        position_vector = position_vector.to(self.Mpch).value

        logging.info(
            "box_size for delta_z: {}".format(
                self.get_comoving_distance_per_delta_z(
                    self.redshift, self.delta_redshift
                ).to(self.Mpch)
            )
        )
        minimum_distance = self.minimum_distance
        logging.info("minimum distance: {}".format(
            minimum_distance.to(self.Mpch)))
        logging.info(str(position_vector.shape))
        shift_array = np.array(
            [
                minimum_distance.to(self.Mpch).value,
                -self.box_size[1].to(self.Mpch).value / 2.0,
                -self.box_size[2].to(self.Mpch).value / 2.0,
            ]
        )
        position_vector = position_vector + shift_array

        x, y, z = position_vector[:,
                                  0], position_vector[:, 1], position_vector[:, 2]
        intensity_catalog = treecorr.Catalog(x=x, y=y, z=z, k=intensity)
        random_catalog = treecorr.Catalog(
            x=x, y=y, z=z, k=mean_intensity_per_redshift_mesh
        )

        if tracer2 is not None:
            intensity_catalog_2 = treecorr.Catalog(x=x, y=y, z=z, k=intensity2)
            random_catalog_2 = treecorr.Catalog(
                x=x, y=y, z=z, k=mean_intensity_per_redshift_mesh2
            )
        logging.info("Done initializing catalogs.")
        parallel_bins = np.linspace(-max_sep, max_sep, 2 * nbins + 1)
        final_correlation = []
        final_correlation_DD = []
        final_correlation_DR = []
        final_correlation_RD = []
        final_correlation_RR = []
        N = 2 * nbins
        for i, rpar_min, rpar_max in zip(
            range(len(parallel_bins) -
                  1), parallel_bins[:-1], parallel_bins[1:]
        ):
            config = {
                "bin_type": "Linear",
                "min_sep": 0,
            ...
            DD = treecorr.KKCorrelation(config)
            DR = treecorr.KKCorrelation(config)
            RD = treecorr.KKCorrelation(config)
            RR = treecorr.KKCorrelation(config)
            logging.info(f"Done initializing correlation function {i+1}/{N}.")
            DD.process(intensity_catalog, intensity_catalog_2)
            DR.process(intensity_catalog, random_catalog_2)
            RD.process(random_catalog, intensity_catalog_2)
            RR.process(random_catalog, random_catalog_2)
            final_correlation.append((DD.xi - DR.xi - RD.xi + RR.xi) / RR.xi)
            final_correlation_DD.append(DD.xi)
            final_correlation_DR.append(DR.xi)
            final_correlation_RD.append(RD.xi)
            final_correlation_RR.append(RR.xi)
            logging.info(f"Done processing correlation function {i+1}/{N}.")

        return (
            DD.meanlogr,
            parallel_bins[:-1] + 0.5 * np.diff(parallel_bins),
            final_correlation,
            final_correlation_DD,
            final_correlation_DR,
            final_correlation_RD,
    ...
    def galaxy_galaxy_correlation(
        ...
        config = {
            ...
            "nbins": nbins,
            "metric": "Euclidean",
            "bin_slop": 0.0,  # 1 / ((max_sep - min_sep)/nbins)
        }
        ...
        if galaxy_selection == "all":
            ...
            mask = np.random.randint(
                low=0, high=self.cat["luminosity"].size, size=1000)
        else:
            raise ValueError(
                "galaxy_selection must be either 'all' or 'detected' or 'undetected'."
            )

        x = self.cat[position][mask][:, 0]
        y = self.cat[position][mask][:, 1]
        z = self.cat[position][mask][:, 2]
        try:
            x = x.to(self.Mpch).value
            y = y.to(self.Mpch).value
            z = z.to(self.Mpch).value
        except AttributeError:
            pass
        N_gal = len(x)
        logging.info(f"Using {N_gal} galaxies.")
        galaxy_catalog = treecorr.Catalog(x=x, y=y, z=z)
        del x, y, z
        if galaxy_selection == "all":
            ...
            kernel_size = int(
                np.round((100 * self.Mpch) / self.voxel_length[0]))
            if kernel_size % 2 == 0:
                kernel_size += 1
            mean_ngal_per_redshift_factor = (
                np.mean(self.n_gal_mesh, axis=(1, 2))
                / np.mean(self.n_gal_mesh)
                / self.N_mesh[0]
            )
            mean_ngal_per_redshift_factor = median_filter(
                mean_ngal_per_redshift_factor, kernel_size
            )
            random_x = []
            for i in range(self.N_mesh[0]):
                x_min = self.voxel_length[0] * i + 0.5 * self.voxel_length[0]
                x_max = x_min + self.voxel_length[0]
                N_gal_x = np.round(
                    N_gal * mean_ngal_per_redshift_factor[i]).astype(int)
                logging.debug("N_gal_x ({}) = {}".format(i, N_gal_x))
                logging.debug("x_min ({}) = {}".format(
                    i, x_min.to(self.Mpch).value))
                logging.debug("x_max ({}) = {}".format(
                    i, x_max.to(self.Mpch).value))
                random_x.append(
                    np.random.uniform(
                        low=x_min.to(self.Mpch).value,
                        high=x_max.to(self.Mpch).value,
                        size=N_gal_x,
                    )
                )
            random_x = np.concatenate(random_x)
            N_rand = len(random_x)
            random_y = np.random.uniform(
                low=0, high=self.box_size[1].to(self.Mpch).value, size=N_rand
            )
            random_z = np.random.uniform(
                low=0, high=self.box_size[2].to(self.Mpch).value, size=N_rand
            )
            logging.info(f"Using {N_rand} randoms.")
        random_catalog = treecorr.Catalog(x=random_x, y=random_y, z=random_z)
        del random_x, random_y, random_z

        DD = treecorr.NNCorrelation(config)
        DR = treecorr.NNCorrelation(config)
        RR = treecorr.NNCorrelation(config)
        logging.info(f"Done initializing correlation function.")

        DD.process(galaxy_catalog)
        DR.process(galaxy_catalog, random_catalog)
        RR.process(galaxy_catalog)
        logging.info("Done processing correlation function.")
        xi, varxi = DD.calculateXi(RR, DR)

        return DD.meanlogr, xi, varxi
    ...
    def run(self, skip_lognormal=False, save_meshes=False, save_results=True):
        ...
        if self.run_corrfunc["intensity"]:
            # run intensity auto-corrfunc
            # probably in a different process with multiprocessing
            pass
        if self.run_corrfunc["n_gal"]:
            # run intensity auto-corrfunc
            # probably in a different process with multiprocessing
            pass
        if self.run_corrfunc["cross"]:
            # run intensity auto-corrfunc
            # probably in a different process with multiprocessing
            pass
